...
# Note: Prompt templates are now handled by the reward function directly
...
def transform_example(
    ...
    if match_type in ["llm_judge", "llm_judge_remote"]:
        ...
        # Store the template name as-is, the reward function will handle loading
        ...
        if verbose:
            ...
            print(f"[transform_example] LLM Judge ({match_type}) configuration added to extra_info for idx={idx}:")
...
def main():
    ...
    parser.add_argument(
        "--llm_prompt_template",
        default="default",
        help="LLM prompt template to use (default: default). Available: 'default', 'detailed', or custom template string.",
    )
    parser.add_argument(
